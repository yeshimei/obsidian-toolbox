...
import { encryptionNote } from 'src/Commands/encryption';
import { insertString, isNoteEncrypt, isResourceEncrypt } from 'src/helpers';
...
test.add('Encryption', '图片笔记', async (self: Toolbox) => {
  ...
  await encryptionNote(self, file, pass, false);
  const decryptContent = await self.app.vault.read(file);
...
test.add('Encryption', '开启图片压缩', async (self: Toolbox) => {
  ...
  await encryptionNote(self, f1, pass, true);
  ...
  await encryptionNote(self, f1, pass, false);
  ...
  await encryptionNote(self, f1, pass + '1', true);
  ...
  self.settings.encryptionImageCompress = false;
  await encryptionNote(self, f1, pass + '1', true);
  await encryptionNote(self, f1, pass + '1', false);
...
test.add('Encryption', '相同一张图片不会重复加密', async (self: Toolbox) => {
  ...
  await encryptionNote(self, f2, pass, true);
